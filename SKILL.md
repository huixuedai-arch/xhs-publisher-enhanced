...
description: |
  小红书发布助手增强版 - 基于 white0dew/XiaohongshuSkills 增强，支持图文、视频、完整写长文发布流程（含一键排版→草稿箱审核）。
...
- **"下一步"按钮找不到**：需要**滚动到页面底部**等待元素渲染后再点击，脚本已实现自动滚动。

---

## 2026-03-26 优化更新 ( Vivian 教的)

### 发布流程优化

1. **标题**：要吸引人、不断句，参考封面风格
2. **正文 brief**：摘选内容作为 brief，像电影预告片那样吸引用户
3. **话题标签**：选人数多的（几亿/十几亿），围绕 AI
4. **原创声明**：要打开
...
| 4 | 填正文 | 内容填充后检查分段 |
| 5 | 点"一键排版" | |
| 6 | **滚动到页面底部**，点"下一步" | 按钮在页面底部，需滚动才出现在 DOM 中 |
| 7 | 选择模板 | 选喜欢的模板 |
| 8 | **再次滚动到底部**，点"下一步" | 进入发布页面 |
| 9 | 图片编辑：点击"图片编辑"下方的"+"按钮 | 会拉起 Finder，**需要手动操作选择图片** |
| 10 | 填写正文 brief | 摘选片段，像预告片 |
| 11 | 添加话题标签 | 选高热度（几亿/十几亿）|
| 12 | 打开原创声明 | |
| 13 | **点"暂存离开"** | 保存草稿，不是发布！|
...
### 文字配图模式流程（2026-04-03 新增）

| 步骤 | 操作 | 说明 |
|------|------|------|
| 1 | 点"上传图文" tab | |
| 2 | 点"文字配图"按钮 | AI 生成配图功能 |
| 3 | 输入文字内容 | 在文本框输入文案 |
| 4 | 点"生成图片" | 等待 AI 生成配图 |
| 5 | 选择模板 | 默认选"基础"卡片 |
| 6 | 点"下一步" | 进入发布页面 |
