...
## 目标
- 增加 7 项互动能力，并使用不同命令名避免与外部项目同名：用户主页信息、主页笔记列表、回复评论、点赞/取消点赞、收藏/取消收藏。

## 实施步骤
- [x] 在 `scripts/cdp_publish.py` 中新增 `profile-snapshot` 与 `notes-from-profile`。
- [x] 新增 `respond-comment`（支持 `comment_id` / `comment_author` / `comment_snippet` 定位目标评论）。
- [x] 新增 `note-upvote` / `note-unvote` / `note-bookmark` / `note-unbookmark`。
- [x] 在 CLI 中为上述命令增加下划线别名（仅兼容命名风格，不使用外部同名命令）。
- [x] 更新 `README.md` 与 `SKILL.md` 命令说明。
- [ ] 冒烟验证：在已登录测试账号执行上述 7 个命令，确认返回结构与页面行为符合预期。
